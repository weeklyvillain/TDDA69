...
class TestExecutor(unittest.TestCase):
  ...
  def run_test_executor(self, instructions, expected_stack, initial_environment, expected_environment):
    ...
    actual_stack = executor.execute(code, env, return_stack = True)
  ...
  def test_013_make_ref(self):
    ...
    # Added later: tests of stack contents.
    self.assertEqual(len(stack), 2)
    self.assertEqual(stack[0].name, "a")
    self.assertEqual(stack[1].name, "b")
 
  
  def test_014_store(self):
    (stack, env) = self.run_test_executor( [ 

      # Store the value under the name "a". Push value to stack.
      (klib.bytecode.opcodes.PUSH_ENV, {}),
      (klib.bytecode.opcodes.PUSH, {"value": -1.0}),
      (klib.bytecode.opcodes.STORE, { "name": 'a'}),
      
      
      # Store the value -1.5 via a reference. 
      (klib.bytecode.opcodes.PUSH_ENV, {}),
      (klib.bytecode.opcodes.MAKE_REF, { "name": 'b'}),
      (klib.bytecode.opcodes.PUSH, {"value": -1.5}),
      (klib.bytecode.opcodes.STORE, { }),
      
      # Store the value behind "a" in current env, under name "c" indirectly.
      
      # Make a reference to c in current env.
      (klib.bytecode.opcodes.PUSH_ENV, {}),
      (klib.bytecode.opcodes.MAKE_REF, { "name": 'c'}),
      
      # Make a reference to a in current env.
      # Note that this is placed on the stack the same way
      # that the value -1.5 was above.
      (klib.bytecode.opcodes.PUSH_ENV, {}),
      (klib.bytecode.opcodes.MAKE_REF, { "name": 'a'}),
      
      # Store in c (the second ref on stack) the value behind
      # the reference a. Also push the value to stack.
      (klib.bytecode.opcodes.STORE, { })],
                                           [-1.0, -1.5, -1.0],
